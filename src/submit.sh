...
# submit.sh
#-----------
# IMPORTANT - Assumption is that the $SPARK_HOME and $KAFKA_HOME environment variables are already set in the
# system that is running the application
...
# SPARK_MASTER=spark://dataottam-MacBook-Pro.local:7077
 ...
 APP_JAR="unicorn$SCALA_VERSION-1.0.jar"
...
$KAFKA_HOME/libs/kafkaclients-0.8.2.2.jar,$KAFKA_HOME/libs/kafka_2.11-0.8.2.2.jar,$KAFKA_HOME/lib
s/metrics-core-2.2.0.jar,$KAFKA_HOME/libs/zkclient-0.3.jar,./lib/sparkstreaming-kafka-0-8_2.11-2.0.0-preview.jar
$PATH_TO_APP_JAR
 else
 $SPARK_SUBMIT --class $APP_TO_RUN --master $SPARK_MASTER --jars
